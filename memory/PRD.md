...
- **Group Tours — Full Itinerary Editor (May 2026)**: Operations can now edit ALL detail-page content without code changes. Previously, the `GroupTourDetail.jsx` page synthesized highlights/itinerary/hotels/inclusions/exclusions client-side from the destination name — now every section is stored per-package and editable.
  ...
  - `_default_rich_content(destination, nights, image)` generates sensible templates for new/unseeded packages. `_seed_defaults_if_empty` now a) inserts new packages with full rich content, b) backfills any legacy pricing-only docs (first pass), and c) backfills packages missing `itinerary` (second pass using `{$exists:False}` OR `{$size:0}`).
  - Frontend editor (`AdminDashboard/GroupTourEditorSections.jsx`): 6 new reusable sub-components (`Section`, `BulletListEditor`, `ItineraryEditor`, `HotelsEditor`, `InclusionsEditor`, `ParagraphListEditor`) mounted as **6 collapsible cards** inside `GroupToursAdmin.jsx` after the pricing table:
    1. Intro Paragraph + Trip Highlights (textarea + add/remove/reorder bullets)
    2. Itinerary (per-day: Day#, Title, Description, Meals B/L/D checkboxes, Hotel note, Move Up/Down/Delete)
    3. Hotels (name, stars, nights, room type, meal plan, image URL)
    4. Inclusions (grouped categories — Accommodation/Transfers/Sightseeing/Miscellaneous — with rename + add-category + bullets)
    5. Exclusions (bullet list)
    6. What to Expect (multi-paragraph textarea)
  - Each section shows a count badge; all sections are collapsed by default to keep the modal manageable.
  - `GroupTourDetail.jsx` refactored: `buildPackage()` now prefers backend data when present (`deal.highlights`, `deal.itinerary`, `deal.hotels`, `deal.inclusions`, `deal.exclusions`, `deal.what_to_expect`, `deal.intro_paragraph`), falling back to the destination-based templates for legacy packages. Itinerary day object mapped from `{meals, hotel_note}` → `{meal, hotel}` to keep the existing render untouched.
  - Verified end-to-end: PUT with custom `intro_paragraph`, `highlights`, `itinerary`, `hotels`, `exclusions`, `what_to_expect` persisted correctly; Admin editor shows the full 5-day default Baku itinerary with meals + hotel notes pre-filled; public Group Tour Detail page renders all 6 backend highlights + 5-day itinerary with correct meals/hotel notes.
