...
- **Group Tours — Catalog Pickers (May 2026)**: Itinerary days, Hotels, and the Destination field can now be linked to existing entries from the **Activities**, **Hotels**, and **Cities** management catalogs.
  ...
  - New reusable component `AdminDashboard/CatalogPicker.jsx`: searchable modal with image thumbnails, sub-text (city/country/duration or stars), filtered by typeahead, "Change" + "Unlink" actions when something is already linked.
  - `ItineraryEditor.SortableDay`: Each day card now shows a "Linked Activity (from Activities catalog)" picker between the title and description. Selecting an activity auto-fills the day title, wraps the activity description in `<p>` tags into the WYSIWYG editor, and persists `activity_id` + `activity_name` for traceability.
  - `HotelsEditor`: Replaced the free-text "Hotel name" input with a "Linked Hotel (from Hotels catalog)" picker. Selecting a hotel auto-fills name, stars (from `star_rating`), and image (first item from `images[]`). The display name + image remain editable as overrides below.
  - `GroupToursAdmin` Destination field: Replaced the free-text input with a "Pick a city from the Cities catalog…" picker (16 cities loaded from `/api/cities`). The selected city's name persists into the package's `destination` field.
  - Catalog API responses cached per-mount on the frontend so opening the picker on multiple days/hotels doesn't refetch. Verified end-to-end with all three pickers: 14 activities, 7 hotels, 16 cities — search live-filters on each.
