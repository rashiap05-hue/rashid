...
- **Cancel Request (two-step approval) on Trip Confirmation (May 2026)**: Agents can now request a booking cancellation directly from the Trip Confirmation (`BookingDetail`) page; admins/operational team approve or reject before the booking is actually cancelled.
  ...
  - Frontend (`BookingDetail.jsx` + new `BookingDetail/CancelRequestModal.jsx`): Red dashed "Cancel Request" button in the Seller sidebar (only visible when booking status isn't cancelled, no pending cancellation already, and travel date still in the future). Modal enforces a required reason textarea + shows the 3 important-info bullets. Once requested, the page status pill flips to rose "Cancellation Requested", and a full banner shows reason/requester/timestamp. Admins/staff/suppliers see an inline review form with optional note + **Approve Cancellation** (red) / **Reject Request** buttons. On approval, pill turns red "Cancelled" and the banner shows reviewer + timestamp + note.
  ...
  - Verified end-to-end via curl (9 test cases: reason required, future-date guard, duplicate-pending guard, approve flips status to cancelled + clears proposal, reject allows resubmission, approve after re-submit, final state check) and Playwright (agent submits → sees rose banner → admin approves with note → page pill flips to red "CANCELLED" + "Booking Cancelled" banner shows reviewer note + timestamps). Notifications verified: 2 cancel-request notifications created (approved + rejected).
