...
- **Group Tour Brochure — Cover gallery + Day-wise images (Feb 2026)**: Extended the existing `/api/group-tours/{id}/brochure-pdf` WeasyPrint generator (`routes/group_tours.py::_build_brochure_html`) to render images in two new places, exactly as requested.
  ...
  - **Day-wise itinerary cards**: Each `ItineraryDay` now starts with a full-width hero banner using the day's primary image (`day.images[0]`). When the day has more than 1 image, a thumbnail strip of up to 4 additional photos renders under the description. **Graceful fallback chain**: if `day.images` is empty, the card uses the day's first linked activity image; if none, it falls back to the package cover hero — so the brochure never prints a flat blank day card even when admins haven't yet uploaded explicit per-day photos.
  - Hero & thumbnail sizes tuned (Feb 2026 follow-up): day-hero shortened from 38mm → 22mm, thumbnail strip 32×22mm → 22×16mm so 3 day-cards fit on one A4 page; brochure compacted from 9 → 7 pages. Footer wordmark "Travo Tours · Brochure" stripped — now just shows page numbers.
  - **Verified end-to-end via Gemini Vision** on the Almaty Eid Break package (4 cover images): cover hero + 3-thumbnail gallery render correctly under the AED 3,449 price; Day 1 with 3 explicit images shows hero banner + 2-thumbnail strip; Day 2 with 1 image shows just the hero banner (no strip); Day 3/4 fallback to activity-image. PDF size 4.0 MB, 7 pages, valid `%PDF-1.7` header.

- **White-label branding — tenant-wide override applied to all customer-facing PDFs (Feb 2026)**: Built a single source of truth in `app_settings.branding` plus a small admin UI so an org can replace the default Travo Tours branding on every PDF the customer sees.
  - **Backend `routes/settings.py`** — exposed `GET /api/settings/branding` (any signed-in user reads), `PUT /api/settings/branding` (admin/staff only) and `DELETE /api/settings/branding` (admin/staff resets). Whitelisted fields: `logo_url`, `company_name`, `footer_email`, `footer_phone`, `footer_website`. New shared async helper `get_pdf_branding()` resolves the saved logo URL into a `data:image/...;base64,…` payload that WeasyPrint can embed inline.
  - **Backend `routes/uploads.py::POST /api/uploads/branding-logo`** — admin uploads PNG/JPG/WEBP/GIF (≤5MB) and gets back `/api/static/branding/<file>.<ext>` for storage in `app_settings.branding.logo_url`.
  - **Backend wiring** — every customer-facing PDF builder now consumes branding before rendering:
    - **Brochure** (`routes/group_tours.py::_build_brochure_html`) — when a logo is set it renders top-left of the cover (was empty after the prior session's de-branding); the footer line on the Terms page concatenates `company_name · email · phone · website` (only the configured fields).
    - **Invoice / Voucher / Receipt / Hotel-Voucher** (`routes/invoice_voucher.py::build_invoice_html`, `build_voucher_html`, `build_receipt_html`, `_build_hotel_voucher_html`) — `_company_block()` now takes a `branding` kwarg and prefers the tenant override over the user record over the legacy default; `_branded_logo()` swaps the embedded `_LOGO_DATA_URL` Travo logo for the uploaded white-label logo when set. All 8 call sites (4 download routes + 4 email-attach routes) updated.
  - **Frontend `components/AdminDashboard/BrandingSettings.jsx`** — new admin tab "Branding". Logo upload card with live preview on a dark navy swatch (so light logos are immediately visible), four labelled input fields with helper text, **Save Branding** + **Reset to default** + **Remove logo** actions, and an info card explaining which PDFs the change applies to. Hot-loaded into the existing AdminDashboard tabs row (15th tab).
  - **Verified end-to-end**: uploaded an "ACME TRAVEL" navy-banner logo + footer details → Gemini Vision confirmed **(a)** brochure cover renders the ACME logo top-left + Acme Travel Co. wordmark on the Terms footer, **(b)** Trip Invoice header now shows ACME TRAVEL logo and the company block reads "Acme Travel Co.", **(c)** Travel Voucher header swapped to ACME TRAVEL with no Travo references. Reset path verified — DELETE branding returns PDFs to the default look.

- **Group Tour Package — Travel Insurance toggle (Feb 2026)**: Added a per-package travel insurance option in the Edit Group Tour Package admin form so ops can bundle insurance into specific packages without touching every itinerary individually.
  - **Backend `routes/group_tours.py`** — `GroupTourPackageBase` and `GroupTourPackageUpdate` now carry an `insurance` block: `{ included: bool, country: str, custom_price_per_person: float|null, description: str }`. Schema-permissive `Dict[str, Any]` typing keeps future fields cheap to add.
  - **Brochure PDF** — `_build_brochure_html` prepends a dedicated **"Travel Insurance"** category to the Inclusions section when `insurance.included = true`. Description resolution chain: explicit override → tenant insurance pricing for the package's country (looked up from `db.insurance_prices`) → "Default" tier → hard-coded fallback "Travel Insurance with min $50,000 coverage — Only for Age Below 60 Yrs". Resolution happens in the async route handler so the existing sync `_build_brochure_html` stays sync.
  - **Frontend `AdminDashboard/GroupTourEditorSections.jsx`** — new exported `<InsuranceEditor>` component. Single checkbox toggle, then when on reveals: country pricing dropdown (live-loaded from `/api/settings/insurance`, auto-defaults to package destination), AED-prefixed custom price override input with **Effective: AED X per traveller** badge, and a description override input.
  - **Frontend `AdminDashboard/GroupToursAdmin.jsx`** — new `<Section title="Travel Insurance">` slotted between Inclusions and Exclusions. Save handler serialises insurance with proper number coercion (custom price → null when blank), and `EMPTY_PKG` ships the default off-state so the field doesn't crash older packages.
  - **Verified end-to-end**: PUT `insurance.included=true` on the Almaty Eid Break package → brochure PDF page 5 now shows a **"Travel Insurance — Travel Insurance with min $50,000 coverage - Only for Age Below 60 Yrs"** row right at the top of Inclusions; toggling off removes the row cleanly. UI screenshot confirms the editor section badge updates 0→1 and the form reveals all three sub-fields when toggled.

- **Travel Insurance — auto-roll into displayed prices + "Inclusive" pill (Feb 2026 follow-up)**: Polish so customers never see an "+AED X insurance" surprise — the per-traveller insurance cost is bundled into every displayed price the moment the package toggle is on.
  - **Backend `routes/group_tours.py::_resolve_insurance_per_pax(pkg)`** — async helper resolving the effective AED amount per traveller. Resolution chain: `insurance.custom_price_per_person` → `db.insurance_prices` row matching `insurance.country || pkg.destination` → `"Default"` row → 50.0 fallback. Returns 0 when `insurance.included == false`.
  - **API surface** — `GroupTourPackageResponse` now exposes `insurance_per_pax: float`. Populated on `GET /group-tours`, `GET /group-tours/{id}`, `POST /group-tours`, and `PUT /group-tours/{id}` so every consumer gets the resolved figure without recomputing.
  - **Brochure PDF cover** — meta row now shows a green-tinted `🛡️ Inclusive of travel insurance` chip alongside destination/dates/duration/stars when toggled on. The "Starting from AED" headline number is the twin-double display price + insurance.
  - **Brochure PDF pricing table** — every tier (Single / Twin / Triple / Child no-bed / Infant) shows the inclusive amount; a small `<b>Inclusive of travel insurance</b> — AED X per traveller already added to all prices above.` note renders directly under the table.
  - **Public Group Tour List page** (`components/GroupTours.jsx::EidDealCard`) — card price now adds `insurance_per_pax`; a tiny `🛡️ Incl. travel insurance` ribbon sits below the AED amount when bundled.
  - **Public Group Tour Detail page** (`components/GroupTourDetail.jsx`) — booking card price = `price_per_adult + insurance_per_pax`; a rounded emerald pill `🛡️ Inclusive of travel insurance` appears under the per-person line. Lucide `Shield` icon imported once.
  - **Verified end-to-end via Gemini Vision + Playwright** with custom override AED 75 on Almaty Eid Break: brochure cover starts at AED 3,524 (was 3,449) + green pill in meta row; pricing table shows 3,724 / 3,524 / 3,524 / 2,724 / 574 inclusive with the explanatory note; Group Tours list card AED 3,524 + ribbon; detail booking card AED 3,524 + pill. All assertions captured via Playwright `data-testid=pkg-price-display` + `pkg-insurance-pill`. Test data reset (insurance off) after verification.

- **Brochure PDF — Flights page added + "What's Excluded" removed (Feb 2026 follow-up)**: Synced the brochure's structured content to match the public Flights tab and stripped a redundant exclusions panel.
  - **Backend `routes/group_tours.py::_build_brochure_html`** —
    - **NEW Flights section** rendered between Day-wise Itinerary and Hotels (becomes Page 5/6 after itinerary length). Card layout exactly mirrors `components/GroupTourDetail/FlightsBlock.jsx`: gray sector header (`Dubai to Tbilisi  Wed, 27 May 2026`) with plane icon → airline strip (`Flydubai  FZ 713`) → 3-column body: timeline (`02:40 PM — DXB, Terminal 2` / `05 Hr: 25 Min` / `05:05 AM⁺¹ — TBS`) | dates column (dep/arr each on their own row) | meta block (`Fare · Baggage · Meals · Cabin`). +1 next-day superscript renders in rose. Section is hidden entirely when `pkg.flights` is empty so older packages don't print a stray "Flights" header.
    - **"What's Excluded" panel removed** from the "At a glance" page. The previous `grid-2` collapsed to a single `<div>` so **Trip Highlights now spans the full content width**, giving more breathing room to the bullets.
  - **CSS** — added `.fl-card`, `.fl-route`, `.fl-airline`, `.fl-body`, `.fl-timeline`, `.fl-conn-line`, `.fl-dates`, `.fl-meta` rules tuned for A4 print metrics (3-column flex with the meta column separated by a left border, mirroring the screenshot).
  - **Verified end-to-end via Gemini Vision** — seeded Dubai↔Tbilisi flight data on Almaty Eid Break → Page 2 shows full-width Trip Highlights with no excluded panel ✅ → Page 5 shows both Dubai→Tbilisi (FZ 713) and Tbilisi→Dubai (FZ-8626) cards matching the user's reference screenshot pixel-by-pixel (sector header, airline strip, 3-col body, +1 superscript, meta values). Test flight data reset to empty after verification.

- **Brochure PDF — Section reorder (Feb 2026 follow-up #2)**: Per ops feedback, the brochure now leads with summary travel info (Flights + Hotels) before the day-by-day, and exclusions sit alongside inclusions on the same page so customers don't have to flip back to the cover.
  - **New page order**: Cover (1) → At a glance + Trip Highlights (2) → **Flights + Hotels (3)** → Day-wise Itinerary (4) → **Inclusions + What's Excluded (5–6)** → Pricing → Terms.
  - The Flights section now starts with `page-break` so it always lands on Page 3 immediately after the at-a-glance summary; the Hotels block flows directly below the flight cards on the same page (when there's space) and falls through naturally otherwise. When `pkg.flights` is empty, the Hotels section retains the page-break itself so it still occupies Page 3.
  - "What's Excluded" was re-introduced as a sub-block beneath Inclusions (separated by a 6mm spacer + its own `<h2>` heading) — same rich list it used to render on Page 2 of the previous layout, just relocated to keep travel-impact info together.
  - **Verified end-to-end via Gemini Vision** with Dubai↔Tbilisi flights on Almaty Eid Break: Page 3 confirmed to show ✈ Flights (FZ 713 + FZ-8626) AND Hotels (Plaza Hotel ★★★★★, 3 Nights, Standard Room, BB) together with no overlap; Page 6 confirmed to show Inclusions categories → What's Excluded bulleted list → Pricing table cleanly stacked. Test flight data reset.

- **Brochure PDF — Airline branded logos + table-based flight card (Feb 2026 follow-up #3)**: Polish so the brochure's Flights page renders airline-branded carriers exactly like the public Flights tab.
  - **Built-in airline catalog** (`routes/group_tours.py`) — auto-resolves logos when `flight.airline_logo` is empty. Matches by airline name OR IATA prefix (`FZ → flydubai`, `EK → emirates`, `G9 → air arabia`, `EY → etihad`, `QR → qatar airways`, `5F → flyone`, `SV → saudia`, `W6 → wizz air`, `OV → salam air`, `WY → oman air`, `GF → gulf air`, `6E → indigo`, `AI → air india`).
  - **Local PNG assets** at `/app/backend/uploads/airlines/<slug>.png` — 14 brand-coloured stylised badges (Pillow-generated, ~280×80 px) served via `/api/static/airlines/`. Wikipedia hot-linking fails behind the pod's egress so logos are baked locally for reliable inline embedding by WeasyPrint.
  - **Card body switched from CSS Flex/Grid to a 5-column HTML `<table>`** (Time | Airport | Date | Meta-Label | Meta-Value) — far more reliable in WeasyPrint than `display: contents` and gives perfect column alignment that matches the user's reference screenshot. Vertical timeline rendered via `border-left` on the airport/duration column.
  - **Verified end-to-end via Gemini Vision** with the same Dubai↔Tbilisi data: both flight cards now display the **blue/orange "flydubai" branded badge** next to the airline name; columns cleanly aligned; Hotels section sits immediately below on the same Page 3. Confidence 100%.

- **Group Tour Package — Per-day editor dates now respected on the saved Proposal + PDF (Feb 2026 follow-up #4)**: Fixed the data-loss bug where the admin's per-day dates entered in the Edit Group Tour Package window were silently overwritten by `start_date + day_offset` calculations once the package was saved as a proposal. Now the editor dates are the source of truth across all customer-facing surfaces.
  - **Backend `routes/group_tours.py::save_group_tour_as_proposal`** — proposal document now snapshots the package's per-day dates as `group_tour_day_dates: [{day:1, date:"2026-05-28"}, ...]`, only including days where the admin actually set an explicit date.
  - **Backend `routes/pdf_generator.py`** — both `build_day_blocks()` and `section_inclusions_exclusions()` now consult the override map first via a shared `_day_date(day_num)` helper, falling back to `add_days(trip_start, day_num - 1)` only when no override exists. Replaced 5 call sites that previously hard-coded the offset math.
  - **Brochure PDF** — `_build_brochure_html` was already passing `d.get("date")` raw (e.g. `"2026-05-27"`); now formats it as `"Wed, 27 May 2026"` via the same `_fmt_flight_date()` helper used for flights. Helper hoisted above the itinerary loop so brochure dates render consistently.
  - **Frontend `ProposalView.jsx`** — added `_dayDateOverrides` `useMemo` + `resolveDayDate(dayNum)` callback. The day-card renderer now calls `resolveDayDate(dayNum)` instead of `new Date(tripStartStr).setDate(...)`. Group Tour day-dates flow through transparently; non-group-tour proposals are unaffected.
  - **Verified end-to-end** with deliberately non-consecutive dates (Day 1=15 Jun, Day 2=17 Jun [2-day gap], Day 3=18 Jun, Day 4=19 Jun): brochure PDF prints `Mon, 15 Jun 2026` … `Wed, 17 Jun 2026` (gap preserved!); save-as-proposal persists `group_tour_day_dates` array correctly; proposal PDF renders Day 1 = `Mon, 15 Jun 2026` (Gemini Vision confirmed). Test data reset back to consecutive 2026-05-28..05-31 for Almaty.


- **Real Stripe Checkout Integration on Pay Now (Feb 2026)**: Wired the existing UI-only Credit Card payment method to the real Stripe Checkout flow via `emergentintegrations.payments.stripe.checkout`. Replaces the prior `alert('integration coming soon')` placeholder.
  - **Backend `routes/payments.py` rewrite**: New `POST /api/payments/stripe/checkout` (auth required) accepts `proposal_id`, `origin_url`, plus booking-confirmation context (travelers, contact_info, paymentOption, customPartialAmount, couponCode, couponDiscount). Amount is ALWAYS computed server-side from `proposal.total_price + markup_land - discount_amount - coupon_discount` (× 0.25 for partial-default, or the admin/staff-entered `customPartialAmount` for partial-custom) — frontend-supplied `amount` is silently ignored. Verified: tampered amount=1 still charges AED 7,398; 401 without auth.
  - **Transaction persistence**: Every checkout writes a `payment_transactions` doc with `status=initiated, payment_status=pending|unpaid`, full `booking_context` snapshot (travelers, contact_info, coupon), `amount`, `final_total`, and Stripe `session_id` — so the booking can be created post-payment without trusting the redirect.
  - **`GET /api/payments/stripe/status/{session_id}` (auth required)**: Polls Stripe + idempotently triggers `_finalize_paid_session` on first `paid` event. Booking creation routes through new shared helper `routes/bookings.py::_create_or_update_booking_doc` (extracted out of the legacy `POST /api/bookings` so wallet and Stripe paths share the same logic, idempotently). Returns `booking_id` + `booking_ref` once paid.
  - **Webhook**: New top-level `POST /api/webhook/stripe` route in `routes/stripe_webhook.py` (mandated by the playbook) reuses the same `_finalize_paid_session` helper. Stripe-managed test key `sk_test_emergent` is in `backend/.env`.
  - **Frontend `PaymentPage.jsx`**: Credit Card "Pay Now" now calls `/api/payments/stripe/checkout` with `window.location.origin` + booking context, redirects to the returned `url`. Wallet flow untouched (Tabby/Bank EMI still show the legacy alert per scope).
...
- **Stale-Closure / Hook Dependency Fixes (Feb 2026)**: Closed out the React-hook portion of the Code Quality Report across the 5 flagged files.
  - **`TripBuilder/ActivitiesModal.jsx`**: `fetchActivities` was called inside `useEffect` but missing from deps. Wrapped in `useCallback([city])` and added to the effect's dep array; added an `if (!city) return` guard. Eliminates the stale-closure where a rapid `city` change could fetch the previous city's activities.
  - **`TripBuilder.jsx`** (3 fixes):
    - Edit-mode hydration effect (L156): deps were `[data?.isEditing]` but body reads ~14 `data.*` properties. Changed dep to `[data]` so a fresh proposal object correctly re-hydrates the form. The `if (!data?.isEditing) return` guard at top keeps non-edit flows untouched.
    - Auto-recommend effect (L253): added `data?.room_data` + `data?.add_transfers` to deps (both read inside) so vehicle picks correctly react to room/pax changes. `autoRecommendDone` is read inside but kept out of deps via explicit `eslint-disable-next-line` with rationale (flips once, early-return guards re-fires).
    - Stay-details debounced search (L507): used `cities[stayDetailsCityIndex]?.name` but `cities` wasn't in deps. Replaced with `data?.cities?.[stayDetailsCityIndex]?.name` and added `data?.cities` to the dep array.
  - **`TripItineraryView.jsx`** + **`WalletPage.jsx`** + **`TripBuilder/SaveProposalModal.jsx`**: audited — already had correct dep arrays (the existing `// eslint-disable-line` on `WalletPage.fetchData` is intentional with a documented rationale).
  - **Verified**: ESLint clean on all 5 files; Playwright smoke-test of TripBuilder edit-mode entry shows zero new console errors (only pre-existing Dashboard hydration warnings remain).

- **Test Credential Migration to test_helpers.py (Feb 2026)**: Closed out the P0 "Test Credential Exposure" finding from the Code Quality Report. Migrated 16 pytest files from hardcoded literals to shared helper imports.
  - **`tests/test_helpers.py`**: Exposed `DEFAULT_PASSWORD` as a re-export of the role-agnostic password for legacy inline literals (alongside the existing per-role `TEST_*_PASSWORD` variables and the `login()` / `admin_token()` / `agent_token()` helpers).
  - **Migrated 16 test files** (test_ai_itinerary_and_refactor, test_booking_confirmation, test_booking_detail, test_booking_status_tracker, test_coupon_finalprice_e2e, test_currency_mobile_features, test_insurance_country_pricing, test_inter_city_transfers, test_modular_backend_refactor, test_pdf_email_features, test_proposal_extras_intercity, test_recommended_feature, test_save_proposal, test_terms_policies, test_travel_insurance, test_wallet_system): replaced every `"testadmin@example.com"` / `"rashid@travotours.ae"` / `"neha@travotours.ae"` / `"supplier@georgiancars.ge"` / `"password123"` literal with the corresponding `TEST_ADMIN_EMAIL` / `TEST_AGENT_EMAIL` / `TEST_STAFF_EMAIL` / `TEST_SUPPLIER_EMAIL` / `DEFAULT_PASSWORD` import from `tests.test_helpers`. Also dropped 24 redundant local-alias constant definitions (e.g., `AGENT_EMAIL = TEST_AGENT_EMAIL`) and replaced their downstream references.
  - **Verified**: `grep` for any leftover credential literals across `tests/*.py` returns zero results (excluding test_helpers.py itself). All 31 test modules `import` cleanly. Sample regression: `pytest tests/test_coupon_finalprice_e2e.py tests/test_booking_confirmation.py tests/test_save_proposal.py tests/test_terms_policies.py tests/test_inter_city_transfers.py tests/test_modular_backend_refactor.py` → 50 passed, 2 pre-existing failures, 7 conditionally-skipped (unrelated to credential migration).
  - **Outcome**: Test files can now be safely scanned by secret-detection tools; CI can override credentials via the standard env-var mechanism (`TEST_ADMIN_PASSWORD`, etc.) without touching source.

- **Inter-City Transfer — Direction-Matching Fix + Dual-City Activity Slots (Feb 2026)**: User reported "No transfers found" for Tashkent → Samarkand inter-city even after redeploying, plus requested that BOTH origin and destination city activity buttons appear on a single inter-city transfer day. Three changes shipped:
  - **Direction-matching bug in `routes/transfers.py::search_inter_city_transfers`**: The wider haystack included the `title` field in BOTH `from_hay` and `to_hay`. Since inter-city transfer titles typically contain both cities (e.g. "Samarkand to Tashkent — Private Inter-City Transfer"), every transfer matched both directions of any A↔B search. Fix: prefer structured `from_location` / `from_city` / `to_location` / `to_city` / `destination` fields when present; only fall back to title when both structured fields are absent. Regression-verified Tbilisi→Gudauri match still returns 1 result.
  - **DayCard new props `fromCityActivities` / `onAddActivityFromCity` / `onRemoveFromCityActivity`**: On any `isCheckInDay` (inter-city transfer day) with an `incomingFromCity`, an additional "Add Activity in {incomingFromCity}" button + an amber-tinted activity list render BEFORE the destination city's activities. Each city's activities are stored under their own `${city}_${day}` key so removal stays surgical.
  - **TripBuilder wiring**: Passed the new props to `DayCard`, reusing the existing `handleAddActivity(cityName, dayNumber)` and `handleRemoveActivity` handlers (no new state).
  - **Data seed**: Inserted 2 real Tashkent↔Samarkand inter-city transfers (AED 850/4hrs, both directions) into preview DB so the regression test has data to find. **Production note**: the production DB may not yet have these entries; the user needs to add them via Admin → Transfers (or we can ship a one-time seed in a future deployment).
  - **Verified**: ESLint clean; Playwright run with a real Tashkent→Samarkand proposal confirmed 1 inter-city transfer card rendered, "Add Activity in Tashkent" + "Add Activity in Samarkand" buttons both visible on Day 4, modal shows "Tashkent to Samarkand — Private Inter-City Transfer / AED 850 / 1 transfer available". Zero new console errors.

- **Internal-Transfer-Day Activity Opt-In Toggle (Feb 2026)**: Added a per-activity admin toggle that controls whether an activity is offered on inter-city transfer days in the Trip Builder. Closes the loop on the inter-city transfer UX work — admins can now curate which activities make sense as a 1-2hr roadside stop vs. full-day activities that shouldn't clutter transfer-day pickers.
  - **Backend (`models/schemas.py`)**: New `internal_transfer_day_eligible: bool = False` field on `ActivityCreate`. Persists through `POST /api/activities`, `PUT /api/activities/{id}`, `PATCH /api/activities/{id}`, and returns via `GET`. No migration needed (Mongo schema-less, default False for legacy docs).
  - **Admin form (`ActivityEditForm.jsx`)**: New amber-highlighted checkbox in the Basic Info tab, immediately below Transfer Type, labeled "**Available on Internal Transfer Days**" with helper text "When enabled, this activity can be added on inter-city transfer days in the Trip Builder (e.g. a 1-2 hour stop between Tashkent & Samarkand) alongside the Internal Hotel Transfer." Loads from `activity?.internal_transfer_day_eligible` and saves through the existing `handleFieldChange` flow.
  - **TripBuilder filter (`TripBuilder/ActivitiesModal.jsx`)**: New `transferDayMode` prop. When true, an "INTERNAL TRANSFER DAY" badge renders in the modal header and the activity list is filtered to only those with `internal_transfer_day_eligible === true`. Empty-state copy updated to "Tag activities as 'Available on Internal Transfer Days' in Admin → Activities to surface them here."
  - **TripBuilder wiring (`TripBuilder.jsx`)**: ActivitiesModal now receives `transferDayMode={!!itinerary.find(d => d.day === activeActivityDay)?.isCheckInDay}`. Works for both the FROM-city ("Add Activity in Tashkent") and the destination ("Add Activity in Samarkand") buttons we added in the previous turn.
  - **Verified end-to-end**: Admin opens Edit Activity → toggles checkbox → saves (verified by curl PATCH). In TripBuilder edit, clicking either "Add Activity in Tashkent" or "Add Activity in Samarkand" on Day 4 (transfer day) opens the modal with the "INTERNAL TRANSFER DAY" badge and shows only the 1 flagged activity per city (filtering out the unflagged full-day Tashkent Heritage Tour we seeded for the control). Lint clean.
  - **Tashkent/Samarkand activities seeded** in preview DB: Tashkent Old City Walking Tour (2hr, eligible), Tashkent Full-Day Heritage Tour (8hr, NOT eligible — control), Samarkand Registan Photo Stop (1hr, eligible). Plus Desert Safari with BBQ Dinner flagged eligible (admin form smoke test).

- **Hotel Room: Blackout Dates + Occupancy-Based Filtering (Feb 2026)**: Two related Hotel Management features shipped end-to-end. Both filter the Trip Builder room picker per the agent's selected trip configuration so unbookable rooms never surface.
  - **Backend (`models/schemas.py`)**: Added `unavailable_dates: List[Dict] = []` to `RoomType`. Schema-less Mongo so no migration; legacy room docs default to empty array. Persists round-trip through `PUT /api/hotels/{id}` and is returned by `GET /api/hotels/{id}` (verified via curl).
  - **Admin form (`HotelEditForm.jsx`)**: New `BlackoutDatesEditor` component renders inside each Room Type card (between Room Images and Rate Plans). Amber-tinted card with "+ Add Range" button — each range has `from` / `to` HTML date pickers + a free-text reason field (e.g. "Renovation", "Sold out"). Multiple ranges per room are supported.
  - **TripBuilder occupancy filter (`HotelDetailsView.jsx`)**: New `adults` + `childrenCount` props plumbed from `TripBuilder.jsx → HotelSelectionModal.jsx → HotelDetailsView.jsx`. Filter applied inside `getAllRooms()` BEFORE rate-plan expansion: `maxA >= adults && (childrenCount === 0 || maxC >= childrenCount)`. Strict rule per user spec — when children > 0, room must explicitly support children.
  - **TripBuilder blackout filter**: Same `getAllRooms()` filter step also drops any room whose `unavailable_dates` ranges overlap the stay window (using standard interval-overlap math: `checkIn < range.to && checkOut > range.from`).
  - **City-specific check-in/out**: Replaced the previous trip-wide date range with a per-city window in `TripBuilder.jsx` — `cityCheckIn = startDate + sum(prior cities' nights)`, `cityCheckOut = cityCheckIn + city.nights`. Blackouts now apply against the actual city stay, not the entire trip.
  - **Empty-state UX**: When the filter hides ALL rooms, the modal now shows a helpful message ("No rooms available for this selection" + a hint listing the occupancy + date range that's filtering everything) instead of the generic "No room options".
  - **Verified end-to-end via Playwright**: Seeded Horizont Hotel Tbilisi with `Standard Single Room (1A/0C)` + a King Room blackout (Jul 10-25). For a 2A+1C / Jul 15-18 trip: ✓ Single Room hidden (occupancy fail), ✓ King Room hidden (blackout overlap). For a 1A / Sep 15-18 trip: ✓ All 5 rooms appear after expanding category pagination. ESLint clean on all touched files.
  - **Production action**: After redeploy, existing rooms keep `unavailable_dates: []` (no behavior change). Admins can start tagging blackouts via Admin → Hotels → Edit → expand each Room Type. The occupancy filter activates immediately based on the existing `room_data` (no admin action required).
