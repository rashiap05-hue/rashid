...
- **Group Tours — Catalog Pickers (May 2026)**: Itinerary days and Hotels can now be linked to existing entries from the **Activities** and **Hotels** management catalogs.
  ...
  - Catalog API responses cached per-mount on the frontend so opening the picker on multiple days/hotels doesn't refetch. Verified end-to-end: PUT with `activity_id="7284156f…"` + `hotel_id="7262b82b…"` persisted correctly; modal renders 14 activities + 7 hotels, search filters live ("Desert" → 1 result).
