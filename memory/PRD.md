...
- **Group Tours — Multi-activity per day (Feb 2026)**: Each itinerary day can now link **up to 5 activities** (was 1). The first activity stays "denormalised" into the legacy `activity_id` / `activity_name` fields so all existing public renderers (`GroupTourDetail.jsx`, `GroupTours.jsx`) keep working untouched.
  - **Backend** (`routes/group_tours.py`): New `ActivityRef = { id, name, image, sub, duration }` Pydantic model — `image` and `sub` are denormalised at pick-time so the editor can render rich cards without re-fetching the catalog. `ItineraryDay.activities: List[ActivityRef]` added. `_sync_image_fields` extended to also normalise `activities[]` ↔ `activity_id`/`activity_name` on every read AND write — and to auto-promote any legacy single-activity day into a 1-element `activities[]` array on first read. Server enforces a hard cap of 5 entries per day.
  - **Frontend** (`SortableDay`): Single picker replaced with a stack of one `CatalogPicker` per linked activity. Each emerald pill now shows: 9×9 **activity thumbnail** + bold **activity name** + sub-line with **city · country · duration** + Change/× controls — matching the user's screenshot mock. A trailing `+ Add another activity for this day…` picker is only visible when the day has < 5 entries.
  - **Legacy hydration**: A `useEffect` in `SortableDay` looks up any linked activity that's missing `image`/`sub` from the cached `loadActivities()` catalog and patches the day in-memory so previously-saved days (which only stored `id`+`name`) immediately render with full details — values are persisted on the next save.
  - **Payload** (`GroupToursAdmin.jsx`): Itinerary serializer passes the full `activities[]` (with `image`, `sub`, `duration`) through (with first item mirrored back into legacy `activity_id`/`activity_name`).
  - Verified end-to-end via curl (3 activities sent → re-GET returns 3, legacy fields = first item) and Playwright screenshot (3 emerald pill rows visible on Day 1 each with thumbnail + sub-line `Almaty · Kazakhstan · X hrs` + Change/×; `+ Add another activity` empty picker still visible below).
